...
public class StatsRepositoryMongoImpl implements StatsRepository {
	...
	private LongTextStats getLongTextStats(@NonNull final LongTextDTO longTextDTO, @NonNull final String formId) {
		Aggregation agg = newAggregation(match(new Criteria("formId").is(formId)),
				match(new Criteria("questionId").is(longTextDTO.getId())),
				count().as("value"));
		AggregationResults<RepliesCount> aggResult = mongoTemplate.aggregate(agg, LongTextReply.class, RepliesCount.class);
		RepliesCount repliesCount = aggResult.getUniqueMappedResult();
		...
		longTextStats.setRepliesCount(repliesCount.getValue());
